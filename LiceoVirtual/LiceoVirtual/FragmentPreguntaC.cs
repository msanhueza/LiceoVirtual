...
{
	public class FragmentPreguntaC : Fragment
	{
		...
		{
			var view = inflater.Inflate(Resource.Layout.FragmentPreguntaC, container, false);
			var myActivity = (Pregunta)this.Activity;
			ListadoPreguntaSolucionItem p = myActivity.getPreguntaActual ();
			var pregunta = view.FindViewById<TextView>(Resource.Id.frag3_pregunta);
			pregunta.Text = p.objPregunta.pregunta;
			var imagen = view.FindViewById<ImageView> (Resource.Id.frag3_imagen);
			if(p.objPregunta.idImagen != -1){
				imagen.SetImageResource (p.objPregunta.idImagen);
			} else {
				imagen.Visibility = ViewStates.Gone;
			}
			var respuestaDesordenada = view.FindViewById<TextView>(Resource.Id.frag3_respuesta_desordenada);
			...
			// SI SE QUIERE OCULTAR UNA PALABRA AL AZAR DE LA FRASE
			/*string palabra = "";
			int indice = 0;
			do{
				Random r = new Random ();
				indice = r.Next (0, respuestaPorPalabra.Count());
				palabra = respuestaPorPalabra[indice].ToUpper();
			}while(palabra.Length<=2);
			respuestaCorrecta = palabra;
			for (int i = 0; i<respuestaPorPalabra.Count(); i++) {
				if (i == indice) {
					for (int j = 0; j < palabra.Length; j++) {
						auxRespuestaDesordenada = auxRespuestaDesordenada + "_ ";
					}
				}
				else{
			...
			for (int i = 0; i<respuestaPorPalabra.Count(); i++) {
				if (i == respuestaPorPalabra.Count () - 1) {
					respuestaCorrecta = respuestaPorPalabra [i].ToUpper();
					for (int j = 0; j < respuestaCorrecta.Length; j++) {
						auxRespuestaDesordenada += " ";
						auxRespuestaDesordenada = auxRespuestaDesordenada + " _";
					}
				} else {
					auxRespuestaDesordenada = auxRespuestaDesordenada + " " + respuestaPorPalabra [i];
				}
			}
			respuestaDesordenada.Text = auxRespuestaDesordenada;
			var respuesta = view.FindViewById<EditText>(Resource.Id.frag3_respuesta);
			Button botonComprobar = view.FindViewById<Button> (Resource.Id.frag3_comprobar);
			EditText editTextRespuesta = view.FindViewById <EditText> (Resource.Id.frag3_respuesta);
			var imagenRespuestaAux = view.FindViewById<ImageView> (Resource.Id.frag3_imagen_respuesta);
			...
			botonComprobar.Click += delegate {
				...
				var imagenRespuesta = view.FindViewById<ImageView> (Resource.Id.frag3_imagen_respuesta);
				if(esCorrecta){
